...
public class GridAndPieceDetection {
    ...
    public static Piece[] imageToPieces(Mat image) {
        ...
        for (int i = 0; i < pieceBlocks.size(); i++) {
            ...
            int[] coords = {(int) Math.round(block.x / abw), (int) Math.round(block.y / abw)};
            blockCoords.add(coords);
        }
        ...
        for (int p = 0; p < 3; p++) {
            ...
            for (int[] coord : piece) {
                pieceBoard.setCell(coord[1], coord[0]); // note that the coordinates are in (y, x) format
            }
